# System Prompt — Esempi per LM Studio

> Questi prompt sono pensati principalmente per **LM Studio**. OpenCode carica già un proprio system prompt di default.

## Quando usarli

- **LM Studio** — scegli il prompt in base al task: codice, analisi, o output strutturato
- **llama-server** — passa il prompt con `--session-load` o come prima message nella chat
- **Altri client** — incolla il testo nel campo "System Prompt" dell'interfaccia

## Regole generali

- **Tutti i prompt in inglese** — i modelli sono addestrati prevalentemente in inglese; istruzioni in italiano possono degradare la qualità
- **Temperature bassa** (0.1–0.5) per task deterministici (JSON, codice). **Temperature media** (0.5–0.7) per analisi o generazione libera
...
**Use case:** programmazione, debug, refactoring, spiegazione codice
...
**System Prompt** — copia e incolla tutto il blocco:
...
**Use case:** analisi codice, estrazione requisiti, documentazione tecnica
...
**Use case:** output JSON/YAML/XML, tool calling, integrazione agentic
...
- **Top-K basso** (10–20) per output sintattico/JSON. **Medio** (30–40) per testo tecnico. Con Min-P attivo, Top-K è meno critico — si può impostare a 0
- **Min-P** preferibile a Top-P per il codice: filtra token improbabili mantenendo coerenza sintattica
- **Repeat Penalty** alta (> 1.1) può rompere pattern ripetitivi legittimi nel codice — usare con cautela
- Per modelli **< 7B**: abbassare Max Tokens a 2048 e semplificare il testo dei prompt
- **I prompt restano in inglese** anche sui modelli multilingue
