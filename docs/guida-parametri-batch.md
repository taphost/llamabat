# Parametri llama-server.exe · Qwen3.6-35B-A3B
...
|---|---|
| `-m %MODEL%` | Path al file GGUF del modello. Carica il modello quantizzato IQ3_XXS (~12.3 GiB, 3.05 BPW) |
| `--mmproj %MMPROJ%` | Path al file `mmproj-F32.gguf` per abilitare la visione. Va usato con `--jinja` e `--chat-template-file`. Per attivarlo, aggiungi il parametro e rimuovi `--no-mmproj` + `--no-mmproj-offload` |
| `--no-mmproj` | Disabilita il proiettore multimodale (visione). Il modello è vision ma non serve per uso testuale, risparmia ~1.66GB VRAM |
...
|---|---|---|
| `-ngl` / `--gpu-layers` | `9999` | Numero massimo di layer da caricare in VRAM. `9999` = "tutto sulla GPU". Con `--fit on` i layer che non entrano vengono automaticamente spostati in RAM |
...
| Parametro | Valore | Descrizione |
|---|---|---|
| `--prio` | `3` | Priorità del processo: -1=low, 0=normale, 1=media, 2=alta, 3=realtime. Realtime garantisce massima CPU durante prefill e generazione |
| `-np` / `--parallel` | `1` | Numero di slot paralleli (richieste simultanee). Con 1 si ottimizza per uso singolo |
...
| Parametro | Valore | Descrizione |
|---|---|---|
| `-b` | `2048` | Batch size logico: numero massimo di token processati insieme nel prefill del prompt. Default: 2048 |
| `-ub` | `512` | Micro-batch size fisico: quanti token processa la GPU in un singolo kernel. Default: 512 |
...
| `--cache-ram` | `512` | Dimensione massima cache in MiB. Default: 8192 MiB |
| `-ctxcp` | `1` | Checkpoint del KV cache. Numero massimo di checkpoint per slot. Default: 32 |
| `--keep` | `-1` | Token del prompt iniziale da preservare durante la compattazione del contesto. `-1` = mantiene intero il system prompt scartando i messaggi più vecchi |
...
| `--cache-idle-slots` | — | Salva e pulisce gli slot inattivi al nuovo task. Richiede `--kv-unified` e `--cache-ram` |
| `--cache-reuse` | `64` | Dimensione minima del chunk per tentare il riutilizzo dal cache via KV shifting, richiede prompt caching abilitato. Default: 0 (disabilitato) |
...
| `--chat-template-file %TEMPLATE%` | Path al file del template Jinja personalizzato. Se non specificato, usa il template dai metadata del modello |
...
| Parametro | Valore | Descrizione |
|---|---|---|
| `--temp` | `0.6` | Temperatura: bilancia creatività e determinismo. 0.6 raccomandato da Qwen per il reasoning. Default: 0.80 |
| `--top-p` | `0.95` | Nucleus sampling: considera i token che coprono il 95% della probabilità cumulativa. Default: 0.95 |
| `--top-k` | `20` | Limita la selezione ai 20 token più probabili. Default: 40, 0 = disabilitato |
| `--min-p` | `0.0` | Soglia minima di probabilità per un token (0 = disabilitato). Default: 0.05 |
| `--presence-penalty` | `0.0` | Penalità per token già presenti nel testo (0 = nessuna). Default: 0.0 |
| `--repeat-penalty` | `1.0` | Penalità per sequenze ripetute (1.0 = nessuna). Default: 1.0 |
...
| Parametro | Valore | Descrizione |
|---|---|---|
| `--reasoning` | `on` | Modalità thinking di Qwen3. `on` = sempre attivo, `off` = disabilitato, `auto` = il modello decide in base alla complessità della richiesta. Default: `auto` |
| `--reasoning-budget` | `-1` | Limite di token per il blocco `<think>`. `-1` = illimitato. Valori positivi (es. `2048`) troncano il thinking |
| `--chat-template-kwargs` | `{"preserve_thinking": true}` | Imposta parametri aggiuntivi per il parser JSON del template, formato JSON valido. Usato per passare opzioni al template |

---

## 11 · Speculative Decoding (ngram-mod)
...
| `ngram-simple` | Cerca nel contesto l'ultimo n-gram corrispondente e usa i m token successivi come draft. Overhead minimo |
...
**Il batch usa `ngram-mod` con questi parametri:**

| Parametro | Valore | Descrizione |
|---|---|---|
| `--spec-type` | `ngram-mod` | Tipo di speculative decoding senza draft model |
| `--spec-ngram-mod-n-match` | `24` | Lunghezza n-gram di lookup (quanti token guardare indietro). Default: 24 |
| `--spec-ngram-mod-n-min` | `48` | Minimo token speculativi per round. Default: 48 |
| `--spec-ngram-mod-n-max` | `64` | Massimo token speculativi per round. Default: 64 |

**Caratteristiche di ngram-mod:**
- Lightweight (~16 MB di memoria costante)
- Complessità e memoria costanti
- Può generare draft di lunghezza variabile (m non è fisso)
- Hash pool condiviso tra tutti gli slot del server

**Statistiche nel log:**
```
draft acceptance rate = 0.57576 (171 accepted / 297 generated)
statistics ngram_mod: #calls = 810, #gen drafts = 15, #acc drafts = 15, #gen tokens = 960, #acc tokens = 730
dur(b,g,a) = 0.149, 0.347, 0.005 ms
```
...
- Tasso >0.5 → guadagno in t/s. Tasso <0.3 → disabilitare o provare un altro tipo
...
| Parametro | Valore | Descrizione |
|---|---|---|
| `--metrics` | — | Abilita endpoint Prometheus su `http://localhost:8000/metrics` |
| `--port` | `8000` | Porta del server HTTP. API compatibile OpenAI su `http://localhost:8000`. Default: 8080 |
...
| Parametro | Valore | Descrizione |
|---|---|---|
| `--log-colors` | `auto` | Abilita colori nei log. `auto` = abilita solo su terminale. Valori: `on`, `off`, `auto` |
| `--warmup` | — | Esegue un warmup con un run vuoto prima di iniziare a servire. Default: abilitato |
